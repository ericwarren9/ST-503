...
standard error and the $R^2$. We are going to fit the model and record these important quantities below. Please note for this problem when we say $R^2$ we are really looking at the adjusted $R^2$.
```{r problem 3-1}
#Make the model
...
# Adjusted R-squared
(fit1R2 <- summary(prostateModel1)$adj.r.squared)
```
...
# Adjusted R-squared
(fit2R2 <- summary(prostateModel2)$adj.r.squared)
```
...
# Adjusted R-squared
(fit3R2 <- summary(prostateModel3)$adj.r.squared)
```
...
# Adjusted R-squared
(fit4R2 <- summary(prostateModel4)$adj.r.squared)
```
...
# Adjusted R-squared
(fit5R2 <- summary(prostateModel5)$adj.r.squared)
```
...
# Adjusted R-squared
(fit6R2 <- summary(prostateModel6)$adj.r.squared)
```
...
# Adjusted R-squared
(fit7R2 <- summary(prostateModel7)$adj.r.squared)
```
...
# Adjusted R-squared
(fit8R2 <- summary(prostateModel8)$adj.r.squared)
```
...
plot(r2,
     se,
     main = "Comparing Adjusted R^2 values to its Standard Error",
     xlab = "Adjusted R^2 Value", 
     ylab = "Standard Error"
     )
abline(lm(se ~ r2), col = "blue")
```

It seems here that as the adjusted $R^2$ gets higher the standard error gets lower. It seems to have this negative linear property with each other. This makes sense given the $R^2$ formula will include the sum of squares residual (and the lower residual amount makes a higher $R^2$).
